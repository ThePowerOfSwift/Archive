...
extension Array where Element: Equatable {
    ...
    */
    ...
    /**
     ...
     >`["a","b","c","d].sorted(withOrderOfContentsIn: ["b","c","d"]) -> ["b","c","d","a"]`
     
     - returns: `Array` of `Elements` sorted with the desired order.
     */
    public func sorted(withOrderOfContentsIn reference: [Element]) -> [Element] {
        
        func appendMatchesOf(_ element: Element, from array: [Element], onto result: [Element])
            -> ([Element],[Element])
        {
            let (matches, leftovers) = array.extractAll(element)
            return (result + matches, leftovers)
        }
        
        func sorted(_ array: [Element],
            withOrderOfContentsIn reference: [Element],
            into result: [Element]
        ) -> [Element]
        {
            guard let (head, tail) = reference.destructured else { return result + array }
            let (sortedArray, remaining) = appendMatchesOf(head, from: array, onto: result)
            return sorted(remaining, withOrderOfContentsIn: tail, into: sortedArray)
        }
        
        return sorted(self, withOrderOfContentsIn: reference, into: [])
    }
    ...
    - TODO: Audit necessity of this. As of Swift 3.0, this is now part of the stdlib.
    */
    ...
    public mutating func replace(_ element: Element, with newElement: Element) throws {
        guard let index = index(of: element) else { throw ArrayError.removalError }
        remove(at: index)
