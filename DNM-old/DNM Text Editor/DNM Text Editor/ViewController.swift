...
class ViewController: NSViewController, NSTextViewDelegate, NSTextStorageDelegate {
    ...
    func updateLines() {
        lines = []
        guard let text = textView.textStorage where text.characters.count > 0 else { return }
        let lineScanner = makeLineScannerWith(text.string)
        var str: NSString?

        while !lineScanner.atEnd {
            let newLineSet = NSCharacterSet.newlineCharacterSet()
            if lineScanner.scanCharactersFromSet(newLineSet, intoString: &str) {
                lineScanner.scanLocation += str!.length
            } else {
    ...
    */
    ...
    /*
    private func makeLineScannerWith(string: String) -> NSScanner {
    ...
    */
    ...
    /*
    func highlightAllLines() {
        let tokenizer = DNMTokenizer()
        let tokenContainer = tokenizer.tokenizeString(textView.textStorage!.string)
        traverseToColorRangeWithToken(tokenContainer, andIdentifierString: "root")
    }
    */
    
    func textDidChange(notification: NSNotification) {
        //textView.setTextColor(defaultTextColor, range: NSMakeRange(0,1))
    ...
    */
    ...
    @IBAction func didPressEnterInPasswordField(sender: NSSecureTextField) {
        ...
        if username.characters.count < 1 {
            print("username must have > 0 characters")
            return
        }
        ...
        case "SIGN UP":
            ...
            do {
                ...
                print("could not sign up user: \(error)")
            }
        case "SIGN IN":
