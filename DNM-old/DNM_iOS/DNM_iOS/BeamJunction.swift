...
public class BeamJunction: CustomStringConvertible {
    ...
    )
    {
        ...
        makeDefaultComponentsOnLevel() // setDefaultComponentsOnLevel
    ...
    // TODO: make safer; make private when things are cleaned up; change to setDefault...
    public func makeDefaultComponentsOnLevel() {
    ...
    // make private again when renderInInterval is cleaned up
    public func setDefaultBeamletDirection() {
...
public func BeamJunctionMake(durationNode: DurationNode) -> BeamJunction {
    ...
    let cur = durationNode.durationInterval.duration.subdivisionLevel!
    let prev: Int? = (durationNode.leafLeft as? DurationNode)?.durationInterval.duration.subdivisionLevel
    let next: Int? = (durationNode.leafRight as? DurationNode)?.durationInterval.duration.subdivisionLevel
