    ...
    */
    ...
    private func createLigatureSpans() {
        ...
        for (s, systemLayer) in systemLayers.enumerate() {
            for (id, dmNode) in systemLayer.dynamicMarkingNodeByPerformerID {
    ...
    func addLigatureComponentsToDMNodes() {
        ...
        for (id, dmLigatureSpans) in dmLigatureSpansByID {
            for dmLigatureSpan in dmLigatureSpans {
                ...
                let lastDMNode = startSystem.dynamicMarkingNodeByPerformerID[id]!
                ...
                let firstDMNode = stopSystem.dynamicMarkingNodeByPerformerID[id]!
                ...
                for s in dmLigatureSpan.systemStartIndex! + 1..<dmLigatureSpan.systemStopIndex! {
                    ...
                    if systemLayers[s].dynamicMarkingNodeByPerformerID[id] == nil {
                        // create dmNode

                        let dmNode = DynamicMarkingStratum(identifier: id, height: 2.5 * g) // hack
                        ...
                        dmNode.build()
                        systemLayers[s].dynamicMarkingNodeByPerformerID[id] = dmNode
                        
                        // hail mary
                        systemLayers[s].eventsNode.insertNode(dmNode,
                            afterNode: systemLayers[s].performerStratumByID[id]!
                        )
                    }
                }
            }
