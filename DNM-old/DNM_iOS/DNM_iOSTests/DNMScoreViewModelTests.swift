...
class DNMScoreViewModelTests: XCTestCase {
    ...
    func testSetDefaultComponentSpans() {
        let scoreModel = makeScoreModel()
        let profile = ViewerProfile(allViewers: [ViewerOmni], currentViewer: ViewerOmni)
        var scoreViewModel = ScoreViewModel(scoreModel: scoreModel, viewerProfile: profile)
        scoreViewModel.setDefaultComponentSpan()
    }
